...
command -v jq >/dev/null 2>&1 || {
    echo "category_dispatch: jq required (pkg install jq)"
    exit 2
}

# Pretty column formatting if available
_cols() {
...
# Full JSON dump for Android app
if [ "${1:-}" = "--json" ]; then
...
count="$(jq -r --arg c "$cat_id" '
    [.categories[] | select(.id==$c)] | length
' "$ANDRAX_REGISTRY")"

[ "$count" -gt 0 ] || {
    echo "category_dispatch: unknown category '$cat_id'"
    exit 3
}

# Print tools in category
jq -r --arg c "$cat_id" '
