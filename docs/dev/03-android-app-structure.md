...
The Android app (`android-app/`) is a thin, dependency-light front-end that
renders the ANDRAX-style category/tool browser and dispatches execution to the
Termux backend. It performs **no** privileged or security-sensitive operations.
It is a **complete, buildable single-module Gradle project** — `./gradlew
assembleDebug` produces an installable APK (see
...
├── build-notes.md                 # build/signing/CI notes for the app
├── docs/
...
└── src/main/
    ...
    └── res/
        ...
        ├── drawable/ic_launcher_{background,foreground}.xml   # vector adaptive-icon layers
        └── mipmap-anydpi-v26/ic_launcher{,_round}.xml
```

The UI is built programmatically (no XML layouts), so the only resources are the
theme, strings/colors, and an **all-vector adaptive launcher icon** (`minSdk 26`
means every supported device uses adaptive icons — no density PNGs needed).
